...
const TRANSLATIONS = {
    Presentation: {
        loading: 'Ładowanie...',
        notFound: 'Nie znaleziono prezentacji zepsółu.',
        show: 'Pokaż',
        teamRank: 'Pozycja w zespole',
        domainCount: 'Rozkład domen',
        top10: 'Top 10',
        top5: 'Top 5',
        teamRankList: 'Kluczowe Talenty Zespołu',
        domainStrength: 'Siła Domen',
        heatmap: 'Heatmapa Talentów',
        teamWeaknesses: 'Obszary Ryzyka',
        teamWeaknessesDesc: 'Talenty najrzadziej występujące na wysokich pozycjach w zespole, co może wymagać poszukania wsparcia na zewnątrz lub zmiany procesów.',
        ofTeam: 'zespołu',
        noWeaknesses: 'Brak wyraźnych obszarów ryzyka.',
        spofAlerts: 'Ryzyka typu SPOF (Single Point of Failure)',
        spofAlertsDesc: 'Talenty, które posiada w Top 5 tylko jedna osoba w zespole. Ryzyko w przypadku jej nieobecności.',
        noSpof: 'Brak wyraźnych S.P.O.F.',
        soleCarrier: 'Jedyny posiadacz',
        specialistTooltip: 'Osoba z wyraźną dominacją tej domeny',
        domainExecutingDesc: 'Osoby z talentami w tej domenie wiedzą, jak sprawić, by rzeczy zostały zrobione.',
        domainInfluencingDesc: 'Osoby z mocnymi stronami w tej domenie pomagają zespołowi dotrzeć do szerszego grona.',
        domainRelationshipDesc: 'Talenty z tej domeny są ' + "klejem" + ', który spaja zespół i buduje więzi.',
        domainStrategicDesc: 'Osoby z talentami w tej domenie ciągle analizują i uczą się, by lepiej decydować.',
        pts: 'pkt'
    },
    Talents: {
        matrix: 'Matryca',
        domains: 'Domeny',
        profile: 'Profile',
        top10Summary: 'Podsumowanie Top 10'
    }
};

export default function PresentationContent({ token }: { token: string }) {
    const locale = 'pl';
    const t = (key: keyof typeof TRANSLATIONS.Presentation) => TRANSLATIONS.Presentation[key] || key;
    const tt = (key: keyof typeof TRANSLATIONS.Talents) => TRANSLATIONS.Talents[key] || key;
                                                ...
                                                >
                                                    ...
                                                    {hoveredDomain === domain && (
                                                        <div className="domain-tooltip">
                                                            {t(DOMAIN_TOOLTIP_KEYS[domain] as keyof typeof TRANSLATIONS.Presentation)}
                                                        </div>
                                                    )}
                                                </th>
                                            );
